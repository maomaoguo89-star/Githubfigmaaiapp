...
import { 
  ...
  Map as MapIcon, X, Save, FolderOpen
} from 'lucide-react';
...
export const InfiniteCanvas = () => {
  ...
  const [selectedNodeForToolbar, setSelectedNodeForToolbar] = useState<string | null>(null);
  ...
  const handleExport = () => {
    ...
    exportToFile(data, `project-${Date.now()}.canvas`);
  };

  // Download node content (image/video)
  const handleDownloadNode = (node: Node) => {
    if (!node.previewUrl) {
      alert('暂无内容可下载');
      return;
    }
    ...
    const extension = node.type === 'video' ? 'mp4' : 'png';
    link.download = `${node.title}-${node.id}.${extension}`;
    
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    
    console.log(`✅ 已下载: ${node.title}`);
  };

  // Handle AI image generation
  const handleGenerateImage = async (prompt: string, params: any) => {
    ...
    try {
      ...
      setNodes(prev => prev.map(n => 
        n.id === editingNode.id 
          ? { ...n, content: prompt, status: 'generating' as const }
          : n
      ));
      ...
      setNodes(prev => prev.map(n => 
        n.id === editingNode.id 
          ? { ...n, previewUrl: mockImageUrl, status: 'done' as const }
          : n
      ));
      
      // Update editing node reference
      setEditingNode(prev => prev ? { ...prev, previewUrl: mockImageUrl, status: 'done' } : null);
  ...
  // Update auto-save when nodes/connections change
  useEffect(() => {
    if (autoSaveRef.current) {
      autoSaveRef.current.saveNow(() => ({ nodes, connections }));
    }
  }, [nodes, connections]);
    ...
    <div className="relative w-full h-screen bg-black overflow-hidden font-sans flex text-white">
      ...
      <div className="relative flex-1 h-full overflow-hidden flex flex-col">
        ...
        <Stage
          ...
          onClick={(e) => {
            // Close editor when clicking on empty canvas area
            if (editingNode && e.target === e.currentTarget) {
              setEditingNode(null);
            ...
            {nodes.map((node) => (
              <Group
                ...
                onClick={(e) => {
                  ...
                  setSelectedNodes([node.id]);
                  // Open editor panel for image/video nodes
                  if (node.type === 'image' || node.type === 'video') {
                    setEditingNode(node);
                    setSelectedNodeForToolbar(node.id);
                  } else {
                    setSelectedNodeForToolbar(null);
                  }
                }}
        ...
        {/* --- Node Inline Editor (for selected image/video nodes) --- */}
        {editingNode && (
          <NodeInlineEditor
            node={editingNode}
            scale={scale}
